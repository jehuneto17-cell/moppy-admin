...
- **Corrigido:** os dois hooks agora guardam `{ uid, profile, loading }` num state só, e tratam `uid` atual ≠ `uid` do último dado carregado como "ainda carregando" — fecha a brecha de um render com dado desatualizado.
...
**2026-09-16 — Auditoria completa de pagamento/cobrança (mobile + admin) — 5 bugs reais corrigidos**

Jehu pediu uma verificação completa do fluxo de dinheiro nos dois repos + o que melhorar na UX. Rodada como investigação separada (agente em background, leu `ESTADO.md` inteiro antes pra não repetir achados antigos), depois corrigido um por um começando pelo dinheiro (UX fica pra próxima sessão/pedido).

1. **Cobrança silenciosa ao escolher faxineira (`candidata.tsx`):** o app chamava `POST /api/orders/{id}/charge` (cobrança imediata quando falta <24h) e ignorava a resposta inteira (`.catch(() => {})`). O pedido já vira "confirmed" antes disso — se o cartão for recusado, nada muda visível pro cliente, que só ia descobrir horas depois (retry) ou nunca. **Corrigido:** lê a resposta e avisa o cliente na hora (`notify`, cross-platform) se falhar ou cair em retry, sem desfazer a escolha da faxineira (mudança maior, fora de escopo). Commit `ce08457`.
2. **Nenhuma notificação push era enviada, nunca:** `/api/notifications/send` existe desde agosto (Bloco 8) e não era chamado por nada em lugar nenhum do código — confirmado com grep no repo inteiro. **Corrigido:** `lib/notifications.ts` extrai o núcleo do envio (o endpoint HTTP original exige idToken de usuário logado, não dá pra chamar de cron/webhook/lib/payments.ts que não tem "usuário" nenhum fazendo a chamada) e liga em 3 pontos: cobrança falha definitivamente (`chargeOrder`), cancelamento automático por falta de pagamento (`cron/reconcile`), e saque falhou/concluído (webhook de transferência, item 3 abaixo). Disputa/chargeback ainda não notificam — fica pra quando entrar na parte de UX. Commit `88b10b6`.
3. **Saque com falha na transferência PIX sumia da carteira sem volta:** o saldo é debitado assim que o Asaas aceita criar a transferência (otimista) mas o webhook só tratava eventos `PAYMENT_*` — nenhum case pra `TRANSFER_FAILED`/`TRANSFER_CANCELLED`/`TRANSFER_DONE`. Se a chave PIX fosse inválida ou o banco recusasse, o dinheiro saía do saldo e nunca voltava sozinho. **Corrigido:** `POST /api/wallets/withdraw` agora grava um doc em `transfers/{asaas_transfer_id}` (única forma do webhook saber de quem é a transferência, já que esse evento não carrega order_id nenhum); o webhook detecta payload de transferência (`transfer` em vez de `payment`) e credita de volta + marca `withdraw_history` como "failed" em caso de falha, idempotente por `asaas_event_id`. Commit `529dbe2`.
4. **Cliente Asaas duplicado quando o cartão é recusado na 1ª tentativa (`app/api/cards/tokenize/route.ts`):** `asaas_customer_id` só era salvo no Firestore depois do `try` inteiro (criar cliente + tokenizar) dar certo — se `tokenizeCard` falha logo depois de `createCustomer` ter sucesso, o id fica só na variável local, próxima tentativa cria outro cliente do zero. Não perde dinheiro, mas suja a conta Asaas. **Corrigido:** salva `asaas_customer_id` assim que `createCustomer` retorna, antes de tokenizar. Commit `3dd3dc8`.
5. **Upload de foto na disputa sem tratamento de erro** (`(client)/pedido/[id]/disputa.tsx` e `(cleaner)/.../disputa.tsx`): mesmo bug do KYC (já corrigido hoje mais cedo) — `handleAddPhoto` sem `catch`, falha de upload deixava o slot vazio sem mensagem nenhuma. Corrigido com o mesmo padrão (`catch` + `Alert`) nos dois arquivos. Commit `59d3b9c`.

**Pendente pra próxima sessão (parte de UX da auditoria, ainda não iniciada):**
- Cliente/faxineira não têm nenhuma forma de cancelar o próprio pedido no app (`cancelOrder` já tem a regra toda pronta, só é acionável pelo admin)
- Tela de pedido do cliente não mostra nada sobre status do pagamento
...
**2026-09-16 — Resto da UX da auditoria de pagamento: todos os itens concluídos**
- Jehu pediu "termina todas". Os 4 itens restantes:
1. **Status de pagamento visível (cliente):** tela do pedido lê `payments/{orderId}` (regra já liberava leitura pro `client_id`) e mostra um badge com o status em português + cor por gravidade (13 estados mapeados — pendente, confirmado, falhou, retry, estornado, disputa, etc). `moppy-mobile` commit `9ae72ad`.
2. **Excluir cartão salvo:** `useCards.ts` ganhou `deleteCard()` (`deleteDoc` direto, `firestore.rules` já libera write pro dono — sem precisar de endpoint). Ícone de lixeira no perfil do cliente, confirmação inline avisando que cartão vinculado a pedido pendente pode fazer a cobrança falhar. `moppy-mobile` commit `79b0f52`.
3. **Notificação de disputa/chargeback** (item que tinha ficado de fora da primeira rodada de notificações): `POST /api/disputes` avisa a faxineira que tem 24h pra responder; `disputes/[id]/resolve` avisa cliente e faxineira da decisão; webhook `PAYMENT_CHARGEBACK_REQUESTED` avisa a faxineira que o saldo congelou. `moppy-admin` commit `333055f`.
4. **Painel Financeiro com pendências:** reaproveita os dados já buscados na página (sem leitura extra, exceto a coleção `transfers` nova) pra montar um alerta no topo — cobrança falhou/retry, estorno falhou, chargeback ativo, saque parado >2h sem confirmação. Só aparece quando tem algo. Precisou de regra nova no Firestore (`match /transfers/{id}`, leitura só admin — **publicada em produção com autorização explícita do Jehu**, `firebase deploy --only firestore:rules`, não mexeu em nenhuma regra existente). `moppy-admin` commit `57027a6`.
- `tsc --noEmit` limpo em todos. **Nada testado ao vivo pela UI nesta rodada** (exigiria login de admin real pro painel financeiro, e um pedido de verdade com cartão/estorno pros outros três) — só leitura de código cuidadosa + typecheck, seguindo o mesmo padrão dos itens já testados antes.
- **Auditoria de pagamento (bugs de dinheiro + UX) considerada fechada.** Item que ficou de fora de propósito, por não ser "dinheiro" nem ter sido pedido: texto "cai em 1-2 dias úteis" no saque (a confiabilidade disso já melhorou com o webhook de transferência, mas o texto em si não foi revisado).
